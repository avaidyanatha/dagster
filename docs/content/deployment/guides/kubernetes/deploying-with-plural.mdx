...
<Warning>
    Before you proceed, make sure that your cloud provider CLI is properly configured and updated to the latest version.
    If it is not configured correctly, Plural will fail and won't be able to create resources on your behalf.
</Warning>

Plural uses a GitOps workflow to manage configuration files and deployment state, so you'll need a Git repository to store your Plural configuration. Plural supports GitHub, GitLab, and Bitbucket currently. This repository will contain the Helm charts, Terraform config, and Kubernetes manifests that Plural will autogenerate for you.
